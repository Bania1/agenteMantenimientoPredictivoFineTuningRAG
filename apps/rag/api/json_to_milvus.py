...
Notas:
    ...
    - Los vectores se generan con el modelo de sentence-transformers 'all-MiniLM-L6-v2'.
...
# ---------------------------------------------------------------------------
# Embedding por SentenceTransformers (búsqueda semántica real)
# ---------------------------------------------------------------------------
...
def milvus_payload(
    ...
    # encode_documents para indexar
    vector = embedding_fn.encode_documents([text_for_vector])[0].tolist()
...
def write_milvus(
    ...
    try:
        ...
        for start in range(0, total, batch_size):
            batch = [
                milvus_payload(r, embed_dim, present_varchar, present_int, embedding_fn)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"Dimensión del vector. Por defecto: {DEFAULT_EMBED_DIM}",
    )
    parser.add_argument(
        "--append",
        action="store_true",
